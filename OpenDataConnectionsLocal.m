...
function Connections=OpenDataConnectionsLocal(Url)
    ...
    elseif ~strfind(Url.Base,'file://')
        msg='Local model Url.Base must start with "file://"';
        if ll
            fprintf(msg);
        else
            SetUIStatusMessage(msg)
    ...
    fig=findobj(0,'Tag','MainVizAppFigure');
    sheet=getappdata(fig,'VariablesTable');  % this is the sheet name to read, not the sheet data!
    [~,~,C] = xlsread(ff,sheet);
    [m,n]=size(C);
    vars=C(1,:)';
    for i=1:n
